 ...
 * 
 */
@Repository("OrderCommonAddressDao")
public class OrderCommonAddressDaoImpl extends BaseDaoImpl<OrderCommonAddress> implements
OrderCommonAddressDao {

	@Override
	public List<OrderCommonAddress> findAddress(int userId) {
		String sql = " select b.ParaName+c.ParaName+u.OAddress OAddress,"
				...
				+ " WHERE u.UserId =" + userId + " order by OAddressID DESC";
	...
	public List getDefAddess(String userId, String defAddess) {
		StringBuffer sql= new StringBuffer(" select a.ParaName Province,b.ParaName City,c.ParaName Area,u.Address Address,u.Consignee,u.Mobile,u.Telephone from T_OrderCommonAddress u  " );
	...
	public List findSetAddress(String oAddressId) {
		...
		sql.append(" LEFT JOIN (select paravalue,ParaName from T_Param where onlygroupflag ='Area') b on u.OCity = b.paravalue " );
		sql.append(" LEFT JOIN (select paravalue,ParaName from T_Param where onlygroupflag ='Area') c on u.OArea = c.paravalue where 1=1 " );
		if (StringUtil.isNotEmpty(oAddressId)) {
			sql.append(" and  u.OAddressID =").append(oAddressId).append(" ");
		}
		Query query = getSession().createSQLQuery(sql.toString());
		List list =query.setResultTransformer(CriteriaSpecification.ALIAS_TO_ENTITY_MAP).list();
		return list;
